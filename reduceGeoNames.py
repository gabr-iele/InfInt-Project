...
def reduceGeoNames(nameFileIn, nameFileOut):
    ...
    for row in file_in:
        row = row.strip().split('\t')
        tot_rows = tot_rows + 1
        if(row[7] == 'MNMT' or row[7] == 'CSTL' or row[7] == 'ANS' or row[7] == 'CH' or row[7] == 'HSTS' or row[7] == 'MUS' or row[7] == 'PAL' or row[7] == 'PYR'): #monuments
...
reduceGeoNames('allCountries.txt', 'reducedGeoNames.txt')
